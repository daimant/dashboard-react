...
const dictDescriptionAbout: { [key: string]: string } = {
  id1: 'Показатель, определяющий процент выполнения запросов без нарушения КС (контрольного срока).',
  id2: 'Показатель, определяющий процент выполнения запросов без нарушения МС (минимального контрольного срока).',
  id3: 'Показатель, определяющий процент запросов, выполненных без возврата на доработку (или ошибочным ' +
    'возвратом на доработку) пользователем.',
  id6: 'Динамика по выполненным запросам без ШК или КЭNULL.',
  id7: 'Динамика по выполненным запросам с ШК, которые не привязаны к договору.',
  id8: 'Количество ШК без привязанной группы сопровождения. Фильтры не учитываются.',
  id10: 'Динамика по среднему времени выполнения запроса. При расчёте среднего времени не учитываются регламентные работы, запросы на ремонт\\установки нового РМ и установки Этран.',
  id11: 'Динамика по штрафам, возвратам и фроду.',
};
